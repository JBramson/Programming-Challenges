 ...
 * Part 2: Find the first character where it and the preceding 13 share no chars in common.
 * Part of me learning Rust.
 */
use crate::helpers::RunMode;

const MAX_SLICE_SIZE_PART_1: usize = 4;
const MAX_SLICE_SIZE_PART_2: usize = 14;
...
pub fn part_2_solution(input_strings: Vec<String>, run_mode: RunMode) -> String {
    ...
    let mut i = MAX_SLICE_SIZE_PART_2 - 1; // Check from the last character of the slice
    while i < master_string.len() {
        match find_last_repeated_char_location(master_string, i, MAX_SLICE_SIZE_PART_2) {
            Some(x) => i = x + MAX_SLICE_SIZE_PART_2,
            None => return (i + 1).to_string(),
        }
    }
