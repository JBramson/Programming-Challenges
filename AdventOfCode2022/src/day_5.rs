...
fn move_top_crates(stacks: &mut Vec<Vec<char>>, quantity: u8, origin_stack: usize, target_stack: usize) {
    for i in 0..quantity {
...
pub fn part_1_solution(input_strings: Vec<String>, run_mode: RunMode) -> String {
    ...
    for mut line in input_strings {
        if line.len() == 0 {
            ...
            move_top_crates(&mut stacks, move_instructions[0].parse::<u8>().unwrap(),
...
pub fn part_2_solution(input_strings: Vec<String>, run_mode: RunMode) -> String {
    let mut final_tops = String::from("");
